...
public:
    ...
    /// an invalid \p prim
    explicit MjcPhysicsTendon(const UsdPrim& prim=UsdPrim())
        : UsdTyped(prim)
    {
    }
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateMjcSideSitesIndicesAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // MJCPATHSEGMENTS 
    // --------------------------------------------------------------------- //
    /// For spatial tendons, this holds the index of the segment each tendon path wrap point belongs to.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateMjcPathSegmentsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // MJCPATHDIVISORS 
    // --------------------------------------------------------------------- //
    /// For spatial tendons, this represents an indexed array of divisors. A tendon path segments' length contribution to the overall tendon length is divided by its divisor.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateMjcPathDivisorsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // MJCPATHCOEF 
    // --------------------------------------------------------------------- //
    /// For fixed tendons passing through joints this represents a multiplicative factor on the position or angle of the targeted joint.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateMjcPathCoefAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    // LIMITED 
    // --------------------------------------------------------------------- //
    /// If true, the tendon length is limited to the range specified by mjc:range:min/max.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateLimitedAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateActuatorFrcLimitedAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // RANGEMIN 
    // --------------------------------------------------------------------- //
    /// Minimum allowed tendon length. Setting this attribute without specifying limited is an error, unless autolimits is set in compiler.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateRangeMinAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // RANGEMAX 
    // --------------------------------------------------------------------- //
    /// Maximum allowed tendon length. Setting this attribute without specifying limited is an error, unless autolimits is set in compiler.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateRangeMaxAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // ACTUATORFRCRANGEMIN 
    // --------------------------------------------------------------------- //
    /// Minimum range for clamping total actuator forces acting on this tendon. See Force limits for details. The compiler expects the lower bound to be nonpositive. Setting this attribute without specifying actuatorfrclimited is an error if compiler-autolimits is “false”.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateActuatorFrcRangeMinAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // ACTUATORFRCRANGEMAX 
    // --------------------------------------------------------------------- //
    /// Maximum range for clamping total actuator forces acting on this tendon. See Force limits for details. The compiler expects the upper bound to be nonnegative. Setting this attribute without specifying actuatorfrclimited is an error if compiler-autolimits is “false”.
    ///
    /// | ||
    /// | -- | -- |
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateActuatorFrcRangeMaxAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateSolRefLimitAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform double[] mjc:solimplimit = [0.9, 0.95, 0.001, 0.5, 2]` |
    /// | C++ Type | VtArray<double> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->DoubleArray |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    MJCPHYSICS_API
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateSolImpLimitAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateSolRefFrictionAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform double[] mjc:solimpfriction = [0.9, 0.95, 0.001, 0.5, 2]` |
    /// | C++ Type | VtArray<double> |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->DoubleArray |
    /// | \ref SdfVariability "Variability" | SdfVariabilityUniform |
    MJCPHYSICS_API
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateSolImpFrictionAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateFrictionLossAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

public:
    // --------------------------------------------------------------------- //
    // WIDTH 
    // --------------------------------------------------------------------- //
    /// Radius of the cross-section area of the spatial tendon, used for rendering in MuJoCo. Parts of the tendon that wrap around geom obstacles are rendered with reduced width.
    ///
    /// | ||
    /// | -- | -- |
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateRgbaAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// the default for \p writeSparsely is \c false.
    MJCPHYSICS_API
    UsdAttribute CreateSpringLengthAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    // MJCPATH 
    // --------------------------------------------------------------------- //
    /// For spatial tendons, this describes a list of sites and geoms the tendon wraps. For fixed tendons, this is instead a list of joints.
    ///
    MJCPHYSICS_API
...
public:
    ...
    // MJCSIDESITES 
    // --------------------------------------------------------------------- //
    /// For spatial tendons, a geom wrapped by the tendon may specify which side of the geom the tendon wraps around via a site prim. This is a list of sites that are used as side sites in mjc:path.
    ///
    MJCPHYSICS_API
